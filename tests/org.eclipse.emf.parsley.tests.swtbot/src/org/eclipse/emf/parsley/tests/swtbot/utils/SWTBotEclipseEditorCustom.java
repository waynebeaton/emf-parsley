...
public class SWTBotEclipseEditorCustom extends SWTBotEditor {
	...
	private void selectProposal(SWTBotTable proposalTable, String proposalText) {
		log.debug("Trying to select proposal {0}", proposalText); //$NON-NLS-1$
	...
	private void selectProposal(final SWTBotTable proposalTable, final int proposalIndex) {
		log.debug("Trying to select proposal with index {0}", proposalIndex); //$NON-NLS-1$
		UIThreadRunnable.asyncExec(new VoidResult() {
			@Override
			public void run() {
				Table table = proposalTable.widget;
				log.debug("Selecting row [{0}] {1} in {2}", proposalIndex, table.getItem(proposalIndex).getText(), //$NON-NLS-1$
						table);
	...
	private SWTBotTable getProposalTable() {
		...
		try {
			...
			log.debug("Found table containing proposals -- {0}", getRows(swtBotTable));
			return swtBotTable;
		} catch (Exception e) {
	...
	private void invokeAction(final String actionId) {
		...
		syncExec(new VoidResult() {
			@Override
			public void run() {
				log.debug("Activating action with id {0}", actionId);
				action.run();
			}
		});
	}
